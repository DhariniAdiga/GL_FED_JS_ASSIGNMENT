document.addEventListener("DOMContentLoaded", function() {
    ...
    function displaySearchResults(results) {
        ...
        if (results.length === 0) {
            ...
            results.forEach(applicant => {
                ...
                `;
                resultsContainer.appendChild(applicantDiv);
            });
        }
    }
    ...
    function handleSearch() {
        ...
        const searchResults = searchApplicants('name', searchTerm);
        displaySearchResults(searchResults);
    }
    function searchApplicants(criteria, value) {
        // Filter applicants based on the selected criteria and value
        const filteredApplicants = applicantsData.filter(applicant => {
          switch(criteria) {
            case 'name':
              return applicant.basics.name.toLowerCase().includes(value.toLowerCase());
            case 'position':
              return applicant.basics.AppliedFor.toLowerCase().includes(value.toLowerCase());
            case 'skills':
              return applicant.skills.keywords.some(skill => skill.toLowerCase().includes(value.toLowerCase()));
            case 'location':
              return Object.values(applicant.basics.location).some(location => location.toLowerCase().includes(value.toLowerCase()));
            case 'education':
              return Object.values(applicant.education).some(edu => 
                Object.values(edu).some(detail => detail.toLowerCase().includes(value.toLowerCase()))
              );
            case 'work':
              return Object.values(applicant.work).some(workDetail => workDetail.toLowerCase().includes(value.toLowerCase()));
            case 'internship':
              return Object.values(applicant.Internship).some(internshipDetail => internshipDetail.toLowerCase().includes(value.toLowerCase()));
            case 'projects':
              return Object.values(applicant.projects).some(projectDetail => projectDetail.toLowerCase().includes(value.toLowerCase()));
            case 'achievements':
              return applicant.achievements.Summary.some(achievement => achievement.toLowerCase().includes(value.toLowerCase()));
            default:
              return false;
          }
        });
